...
# 2. replace main tag
main_start = code.find('    <!-- 主內容區：背景使用極淺灰白 -->')
main_end = code.find('    </main>') + len('    </main>')

new_main = """    <!-- 主內容區：背景使用極淺灰白 -->
...
js_start_str = '            // --- 備忘錄核心邏輯 ---'
js_end_str = '            setInterval(updateCountdown, 60000);'
js_start_idx = code.find(js_start_str)
js_end_idx = code.find(js_end_str) + len(js_end_str)
...
code = code[:js_start_idx] + new_js + code[js_end_idx:]
